# File: Corlett_p2_xc2.py
# Author: Caleb Corlett
# Date: 4-21-2023
# Section: 1502
# E-mail: caleb.corlett@maine.edu
# Description:
""" Use recursion to generate Mondrian style art with modified default rates"""
# Collaboration:
"""                             just me                            """
...
STARTING_HEIGHT = 900
PIXEL_MIN = 20
WITHHOLD_PROBABILITY = 1.0       # Higher is less probable to split
SPLIT_PERCENT_LBOUNDS = 0.48
SPLIT_PERCENT_HBOUNDS = 0.52
YELLOW_RATE = 0.09
BLUE_RATE = 0.25
RED_RATE = 0.95
...
def mondrian(pTopLeft, pWidth, pHeight, pWindow):
 ...
 # Divider Assignment
    if(xSplitCheck < pWidth):
        xDivider = randrange(int(pWidth * SPLIT_PERCENT_LBOUNDS), int(pWidth * SPLIT_PERCENT_HBOUNDS)+1) + pTopLeft.x
    else:
        xDivider = pWidth + pTopLeft.x
    if(ySplitCheck < pHeight):
        yDivider = randrange(int(pHeight * SPLIT_PERCENT_LBOUNDS), int(pHeight * SPLIT_PERCENT_HBOUNDS)+1) + pTopLeft.y
    else:
